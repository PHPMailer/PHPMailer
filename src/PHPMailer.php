...
class PHPMailer
{
     ...
     *
     * @var callable|\Psr\Log\LoggerInterface|string
     */
    ...
    public function __construct($exceptions = null)
    {
        ...
        $this->Debugoutput = (false !== strpos(PHP_SAPI, 'cli') ? 'echo' : 'html');
    }
    ...
    public function preSend()
    {
        if ('smtp' === $this->Mailer or
            ('mail' === $this->Mailer and 0 === stripos(PHP_OS, 'WIN'))
        ) {
        ...
        if (1 === ini_get('mail.add_x_header')
            and 'mail' === $this->Mailer
            and 0 === stripos(PHP_OS, 'WIN')
        ...
        if (array_key_exists(2, $images)) {
            ...
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (preg_match('#^data:(image/(?:jpe?g|gif|png));?(base64)?,(.+)#', $url, $match)) {
                    if (4 === count($match) and 'base64' === $match[2]) {
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
        ...
        foreach ($headers as $header) {
            if (0 === strpos($header, 'From:')) {
                $from_header = $header;
                $current = 'from_header';
            } elseif (0 === strpos($header, 'To:')) {
                $to_header = $header;
                $current = 'to_header';
            } elseif (0 === strpos($header, 'Date:')) {
                $date_header = $header;
                $current = 'date_header';
            } else {
                if (!empty($$current) and 0 === strpos($header, ' =?')) {
                    $$current .= $header;
                } else {
                    $current = '';
                }
    ...
    protected static function isShellSafe($string)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            ...
            if (!ctype_alnum($c) && false === strpos('@_-.', $c)) {
                return false;
            }
        }
    ...
    protected function serverHostname()
    {
        ...
        } elseif (function_exists('gethostname') and false !== gethostname()) {
            $result = gethostname();
